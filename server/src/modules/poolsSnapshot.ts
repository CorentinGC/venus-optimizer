...
export async function runPoolsSnapshot(options?: PoolsSnapshotOptions): Promise<{ marketsPath: string; marketsCount: number; updatedDailyFiles: number }> {
  ...
  function averagesFromPoints(points: Array<{ timestamp?: string; apyBase?: number | null; apyBaseBorrow?: number | null }>): {
  ...
  for (const m of markets) {
    ...
    // Fresh-only: use pools/<symbol>.json generated by this snapshotter
    const fileFresh = path.resolve(outDir, 'pools', `${symbolLower}.json`);
    const arrFresh = await readJsonArray(fileFresh);
    const { avg1m, avg3m, avg6m, avg1y } = averagesFromPoints(arrFresh);
